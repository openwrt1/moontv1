    ...
    const fetchSourceDetail = async (
      ...
      try {
        ...
        if (!detailResponse.ok) {
          ...
          return {
            data: [],
            error: `播放线路 [${source}] 发生异常: ${
              errData.error || '连接超时或被拒绝'
            }`,
          };
        }
    ...
    const initAll = async () => {
      ...
      if (
        ...
        if (detailResult.data && detailResult.data.length > 0) {
          sourcesInfo = detailResult.data;
        } else if (sourcesInfo.length === 0) {
          // 如果详情获取失败且没有搜索结果兜底，才抛出错误
          setError(
            detailResult.error ||
              `未找到该视频，或指定的播放源 [${currentSource}] 已失效`
          );
          setLoading(false);
          return;
        } else {
          console.warn(
            `[Warning] 指定的源 ${currentSource} 已失效，将自动切换为其他可用源`
          );
          setNeedPrefer(true);
  ...
  useEffect(() => {
    ...
    const initPlayer = async () => {
      try {
        ...
        // 创建新的播放器实例
        (Artplayer as any).PLAYBACK_RATE = [0.5, 0.75, 1, 1.25, 1.5, 2, 3];
        (Artplayer as any).USE_RAF = true;

        artPlayerRef.current = new (Artplayer as any)({
